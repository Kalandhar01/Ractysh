...
const slides: WhoWeAreSlide[] = [
  ...
  {
    alt: "Premium global logistics port with cargo containers and shipping cranes",
    ...
    title: ["Global trade systems", "connected through", "enterprise logistics."]
  },
  {
      ...
      <div className="relative z-30 flex h-full items-end justify-center px-5 pb-[14svh] pt-24 text-center sm:px-8 md:items-center md:justify-start md:px-14 md:pb-0 md:pr-32 md:text-left lg:px-24 lg:pr-44 xl:pr-52">
        <AnimatePresence mode="wait" initial={false}>
          <motion.article
            ...
            transition={motionTransition}
          >
            <p className="font-manrope text-[0.68rem] font-bold uppercase leading-none tracking-[0.28em] text-[#e7c778] drop-shadow-[0_2px_14px_rgba(0,0,0,0.65)] sm:text-[0.74rem]">
              {active.category}
            </p>
            <h2 className="mt-4 font-cormorant text-[3.25rem] font-semibold leading-[0.9] tracking-normal text-[#fff8ea] drop-shadow-[0_16px_44px_rgba(0,0,0,0.62)] max-[374px]:text-[2.78rem] sm:mt-5 sm:text-[5rem] md:text-[4.85rem] lg:text-[4.95rem] xl:text-[6rem] 2xl:text-[7rem]">
            ...
            <p className="mx-auto mt-5 max-w-[37rem] font-manrope text-[0.98rem] font-medium leading-[1.8] text-[#fff2d6]/88 drop-shadow-[0_8px_26px_rgba(0,0,0,0.7)] sm:text-[1.06rem] md:mx-0 md:text-[1.12rem]">
              {active.subtext}
            </p>
          </motion.article>
        </AnimatePresence>
...
function CarouselIndicators({
  ...
  return (
    <nav
      ...
      className="absolute right-4 top-1/2 z-40 flex -translate-y-1/2 items-center gap-4 sm:right-7 md:right-10 lg:right-16 xl:right-20"
    >
      <div className="relative h-44 w-px shrink-0" aria-hidden="true">
      ...
      <div className="flex h-44 flex-col items-center">
        {slides.map((slide, index) => {
          const isActive = index === activeSlide;
          ...
          return (
            <motion.button
              ...
              className="group relative grid w-12 flex-1 place-items-center overflow-visible font-manrope text-[0.74rem] font-light leading-none tracking-[0.34em] outline-none focus-visible:outline focus-visible:outline-1 focus-visible:outline-offset-4 focus-visible:outline-[#e7c778]/70"
            ...
            >
              {isActive ? (
                <motion.span
                  ...
                  className="absolute inset-[-1.15rem] rounded-full bg-[radial-gradient(circle,rgba(231,199,120,0.14),rgba(231,199,120,0.045)_34%,transparent_68%)]"
                  transition={indicatorTransition}
                />
              ) : null}
              <span
                className={`relative transition-colors duration-700 ease-in-out ${
                  isActive ? "text-[#fff8ea] drop-shadow-[0_0_12px_rgba(231,199,120,0.34)]" : "text-[#e7c778] group-hover:text-[#fff1c0]"
                }`}
              >
                {slide.number}
              </span>
