...
export function EnterpriseBriefingSubscribe({
  ...
  const inputClass = [
    ...
    isPopup ? "rounded-full pl-10 pr-4" : "rounded-[7px] px-3.5",
    isPopup
      ? "bg-[#fffaf0]/78 text-[#21180f] placeholder:text-[#6d5734]/42"
      : "bg-white/[0.04] text-[#fff8ec] placeholder:text-white/28",
    error
      ? isPopup
        ? "border-[#b68a2f]/65 shadow-[0_0_0_3px_rgba(214,180,95,0.15),0_0_30px_rgba(214,180,95,0.2)]"
        : "border-[#e8c16f]/70 shadow-[0_0_0_3px_rgba(224,197,121,0.12),0_0_30px_rgba(224,197,121,0.16)]"
      : focused || email
        ? isPopup
          ? "border-[#d6b45f]/72 bg-white shadow-[0_0_0_3px_rgba(214,180,95,0.14),0_0_32px_rgba(214,180,95,0.18)]"
          : "border-[#E0C579]/55 bg-white/[0.055] shadow-[0_0_0_3px_rgba(224,197,121,0.10),0_0_28px_rgba(224,197,121,0.12)]"
        : isPopup
          ? "border-[#d6b45f]/24 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)]"
          : "border-white/[0.08]"
  ].join(" ");
  const buttonClass = [
    "relative inline-flex h-11 items-center justify-center gap-2 overflow-hidden whitespace-nowrap border border-[#E0C579]/25 text-[#17130d] shadow-[0_18px_46px_rgba(0,0,0,0.24)] transition duration-300 hover:-translate-y-0.5 hover:border-[#E0C579]/55 hover:bg-[#E0C579] hover:shadow-[0_20px_54px_rgba(0,0,0,0.28),0_0_34px_rgba(224,197,121,0.2)] disabled:cursor-default disabled:hover:translate-y-0",
    isPopup ? "flex-1 rounded-full bg-[#1b150f] px-5 text-[0.84rem] font-semibold normal-case tracking-normal text-[#fff8ec]" : "rounded-[7px] bg-[#f5ebd8] px-4 text-[0.78rem] font-bold uppercase tracking-[0.13em]",
    status === "loading" ? "border-[#E0C579]/60 bg-[#E0C579] text-[#17130d] shadow-[0_18px_52px_rgba(224,197,121,0.2),0_0_42px_rgba(224,197,121,0.22)]" : ""
  ].join(" ");
      ...
      <AnimatePresence mode="wait" initial={false}>
        {status === "success" ? (
          ...
          <motion.form
            ...
            className={isPopup ? "mt-5 space-y-3" : "mt-5"}
            noValidate
          >
            <div className={isPopup ? "space-y-3" : "grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"}>
              <label className={inputShellClass}>
                {isPopup ? (
                  <Mail className="pointer-events-none absolute left-4 top-1/2 z-10 h-4 w-4 -translate-y-1/2 text-[#d6b45f]/78" />
                ) : null}
                <input
                  required
                  ...
                  placeholder={isPopup ? "email@company.com" : "Enter email"}
              ...
              >
                {status !== "loading" ? (
                  <motion.span
                    ...
                    className={`pointer-events-none absolute inset-0 border border-[#E0C579]/28 bg-[radial-gradient(circle_at_50%_50%,rgba(224,197,121,0.22),transparent_62%)] ${
                      isPopup ? "rounded-full" : "rounded-[7px]"
                    }`}
                  />
                ) : null}
                <span
                ...
                <AnimatePresence mode="wait" initial={false}>
                  ...
                  >
                    {status === "loading" ? "Synchronizing..." : "Subscribe"}
                  </motion.span>
                </AnimatePresence>
            ...
            <AnimatePresence>
              {error ? (
                <motion.p
                  ...
                  className="mt-2 text-[0.76rem] font-medium text-[#f0d58a]"
                >
                  {error}
                </motion.p>
              ) : null}
