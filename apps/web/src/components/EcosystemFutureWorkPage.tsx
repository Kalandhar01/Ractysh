...
const importExportConfig: FutureWorkConfig = {
  ...
  mutedLineIndex: 1,
  body:
    "Import, export, cargo coordination and enterprise logistics systems are being expanded into a dedicated global trade ecosystem.",
  status: "Trade Ecosystem Initializing",
  primaryHref: "/",
  Visual: ImportExportLogisticsVisual
};
  ...
  return (
    ...
    >
      ...
      <section className="relative z-10 mx-auto flex w-full max-w-[720px] flex-col items-center text-center">
        ...
        >
          <span className="logistics-status-dot relative h-2 w-2 rounded-full bg-[#d6b45f]" />
...
function ImportExportLogisticsVisual(_props: { reduceMotion: boolean }) {
  return (
    <>
      <div className="logistics-pulse-ring absolute inset-4 rounded-full border border-[#d6b45f]/[0.24]" />
      <div className="logistics-pulse-ring logistics-pulse-ring-delayed absolute inset-8 rounded-full border border-[#fff7e8]/10" />
      ...
      <div className="logistics-wire-grid absolute inset-[2rem] rounded-full opacity-[0.55] sm:inset-[2.4rem]" />
      ...
      {[
        ...
        <span key={position} className={`logistics-node absolute ${position} h-1.5 w-1.5 rounded-full bg-[#fff7e8]`} />
      ))}
    </>
  );
}
...
function DesignArchitectureVisual({ reduceMotion }: { reduceMotion: boolean }) {
  return (
    <>
      <div className="logistics-pulse-ring absolute inset-4 rounded-full border border-[#d6b45f]/[0.22]" />
      <div className="logistics-pulse-ring logistics-pulse-ring-delayed absolute inset-8 rounded-full border border-[#fff7e8]/10" />
      <div className="logistics-wire-grid absolute inset-[1.9rem] rounded-full opacity-[0.46] sm:inset-[2.25rem]" />
      <svg
        viewBox="0 0 180 180"
      ...
      <span className="logistics-node absolute left-[19%] top-[33%] h-1.5 w-1.5 rounded-full bg-[#fff7e8]" />
      {!reduceMotion && (
        <>
          <span className="logistics-node absolute right-[18%] top-[37%] h-1.5 w-1.5 rounded-full bg-[#fff7e8]" />
          <span className="logistics-node absolute bottom-[19%] left-[30%] h-1.5 w-1.5 rounded-full bg-[#fff7e8]" />
        </>
      )}
    </>
  );
...
function InfraExecutionVisual({ reduceMotion }: { reduceMotion: boolean }) {
  return (
    <>
      <div className="logistics-pulse-ring absolute inset-4 rounded-full border border-[#d6b45f]/[0.22]" />
      <div className="logistics-pulse-ring logistics-pulse-ring-delayed absolute inset-8 rounded-full border border-[#fff7e8]/10" />
      <div className="logistics-wire-grid absolute inset-[1.9rem] rounded-full opacity-50 sm:inset-[2.25rem]" />
      <svg
        viewBox="0 0 180 180"
      ...
      {!reduceMotion && (
        <span className="logistics-node absolute right-[22%] top-[28%] h-1.5 w-1.5 rounded-full bg-[#fff7e8]" />
      )}
    </>
  );
}
