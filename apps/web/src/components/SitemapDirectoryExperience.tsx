...
const ecosystemLinks: DirectoryLink[] = [
  ...
  { label: "Export & Import", href: "/import-export", description: "Enterprise trade coordination and logistics systems." },
...
function DirectoryCard({ item, index }: { item: DirectoryLink; index: number }) {
  return (
    ...
    >
      ...
      <div className="mt-8">
        <h3 className="text-[1.55rem] font-semibold leading-tight text-[#1c1510] [font-family:var(--font-cormorant)] md:text-[1.85rem]">
          {item.label}
        </h3>
...
export function SitemapDirectoryExperience({ content }: SitemapDirectoryExperienceProps) {
  ...
  const stats = [
    ...
    { label: "Service Workflows", value: String(servicePages.filter((service) => service.href !== "/book-demo").length), Icon: Building2 },
        ...
        <section className="relative z-10 px-5 pb-14 pt-32 md:px-8 md:pb-16 lg:pt-40">
          <div className="mx-auto max-w-[1080px]">
            <Reveal className="text-center">
              ...
              <h1 className="mx-auto mt-5 max-w-[820px] text-[3.45rem] font-semibold leading-[0.92] text-[#18130f] [font-family:var(--font-cormorant)] sm:text-[4.5rem] md:text-[5.7rem] lg:text-[6.6rem]">
                Sitemap
              </h1>
            ...
            <Reveal delay={0.06} className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {stats.map(({ label, value, Icon }) => (
                <div key={label} className="rounded-[8px] border border-[#d8c18a]/62 bg-[#fffaf0]/76 p-5 shadow-[0_16px_42px_rgba(88,61,28,0.06),inset_0_1px_0_rgba(255,255,255,0.88)]">
                  ...
                  <p className="mt-5 text-[2.2rem] font-semibold leading-none text-[#1b1511] [font-family:var(--font-cormorant)]">{value}</p>
                </div>
              ))}
            </Reveal>
          </div>
                ...
                <section className="scroll-mt-28">
                  <div className="mb-7 grid gap-4 border-y border-[#c8a65d]/24 py-6 lg:grid-cols-[0.42fr_1fr] lg:items-end">
                    <div>
                      ...
                      <h2 className="mt-3 text-[2.4rem] font-semibold leading-none text-[#1b1511] [font-family:var(--font-cormorant)] md:text-[3.2rem]">
                        {group.title}
                      </h2>
                    </div>
        ...
        <section className="relative z-10 px-5 pb-24 md:px-8 lg:pb-32">
          <Reveal className="mx-auto flex max-w-[1000px] flex-col gap-6 rounded-[8px] border border-[#d8c18a]/70 bg-[#f3e8d7]/78 p-6 shadow-[0_24px_70px_rgba(80,52,24,0.08)] md:flex-row md:items-center md:justify-between md:p-8">
            <div>
              ...
              <h2 className="mt-3 text-[2rem] font-semibold leading-tight text-[#1b1511] [font-family:var(--font-cormorant)] md:text-[2.6rem]">
