...
const FLOW_TRANSITION_SECONDS = 0.8;
const power3Out: [number, number, number, number] = [0.215, 0.61, 0.355, 1];
const flowTransition = { duration: FLOW_TRANSITION_SECONDS, ease: power3Out };
...
> = {
  completed: {
    cardClassName: "bg-[#103b2c]/88 text-[#dff9e9]",
    badgeClassName: "border-[#34d37c]/45 bg-[#123f2c]/92 text-[#dff9e9]",
    label: "Completed",
    icon: CheckCircle2,
    dot: "bg-[#34d37c]",
    opacity: 0.9,
  ...
  active: {
    cardClassName: "bg-[#4c3510]/92 text-[#fff4c7]",
    badgeClassName: "border-[#f1c94a]/80 bg-[#f1c94a] text-[#111b2c]",
    label: "Active",
    icon: Clock3,
    dot: "bg-[#111b2c]",
    opacity: 1,
  ...
  future: {
    cardClassName: "pointer-events-none bg-white/[0.055] text-white/56",
    badgeClassName: "border-white/10 bg-black/18 text-white/58",
    label: "Queued",
    icon: LockKeyhole,
    dot: "bg-white/24",
    opacity: 0.54,
...
function latestNote(notes: string | undefined): string {
  if (!notes) return "";
...
function opacityForVisualState(visualState: PresentationStageState): number {
  ...
  if (visualState === "completed") return 0.9;
  return 0.6;
}
...
export function ConsultationWorkflowTracker() {
  ...
  return (
    <section
      ref={sectionRef}
      className="workflow-wrapper relative isolate min-h-[500vh] bg-[#16110d] text-white"
    >
      <div
        ref={stickyRef}
        className="workflow-sticky workflow-gpu sticky top-0 flex h-screen items-center overflow-hidden px-5 py-5 md:px-8 md:py-7"
      >
        <div
        ...
        >
          <div className="grid gap-5 xl:grid-cols-[0.72fr_1fr] xl:items-end">
            <div>
              ...
              <h2 className="mt-3 max-w-[34rem] font-display text-[24px] font-semibold leading-[1.04] tracking-[-0.04em] text-white md:text-[30px] lg:text-[36px]">
                A private path from first brief to execution confidence.
              </h2>
              <p className="mt-4 max-w-2xl text-[15px] leading-[1.7] text-white/70 md:text-[16px]">
                Every submitted brief creates a live workflow record, stage history, admin actions and response
                documents from the operational desk.
              </p>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                ...
                <div
                  ...
                  className={cn(
                    "workflow-indicator-gpu rounded-lg border border-white/10 bg-white/[0.065] p-4 backdrop-blur-xl transition-opacity duration-[800ms] ease-[cubic-bezier(0.215,0.61,0.355,1)]",
                ...
                >
                  <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-white/42">{item.label}</p>
                  <p className="mt-2 min-h-10 text-[14px] font-semibold leading-[1.45] text-white">{item.value}</p>
                </div>
              ))}
            </div>
          </div>

          <div className="mt-7 overflow-hidden rounded-xl border border-white/12 bg-white/[0.07] shadow-[0_34px_110px_rgba(0,0,0,0.32),inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-2xl">
            <div className="flex flex-col gap-4 border-b border-white/10 px-5 py-4 md:flex-row md:items-center md:justify-between">
              ...
              {latestActivity ? (
                <span className="rounded-md border border-white/10 bg-black/22 px-3 py-2 text-[12px] text-white/60">
                ...
                <div className="overflow-x-auto pb-2 pt-4 lg:overflow-visible" data-lenis-prevent>
                  ...
                  >
                    {stages.map((stage, index) => {
                      ...
                      const note = latestNote(stage.adminNotes);
                      ...
                      return (
                        ...
                        >
                          <div
                            ...
                            className={cn(
                              "workflow-card-gpu relative flex min-h-[23rem] flex-col overflow-hidden rounded-lg border p-4 shadow-[0_18px_58px_rgba(0,0,0,0.18),inset_0_1px_0_rgba(255,255,255,0.08)] transition-[transform,opacity] duration-[800ms] ease-[cubic-bezier(0.215,0.61,0.355,1)]",
                              meta.cardClassName
                            )}
                          >
                            ...
                            <div className="relative flex items-start justify-between gap-3">
                              <span
                                data-workflow-indicator
                                className={cn(
                                  "workflow-indicator-gpu flex h-12 w-12 shrink-0 items-center justify-center rounded-lg border border-white/12 bg-black/22",
                                  visualState === "active"
                              ...
                              <span
                                data-workflow-indicator
                                style={{
                                  opacity: visualState === "future" ? 0.82 : 1,
                            ...
                            <div className="relative mt-5 flex items-center gap-2 text-[12px] font-semibold uppercase tracking-[0.06em] text-current/72">
                              <span
                                data-workflow-indicator
                                className={cn(
                                  ...
                                  visualState === "future" ? "opacity-60" : "opacity-100"
                                )}
                              >
                            ...
                            <h3 className="relative mt-3 min-h-12 text-[16px] font-semibold leading-snug md:text-[18px]">
                              {stage.title}
                            </h3>
                            <p className="relative mt-3 text-[13px] leading-[1.65] text-current/72">{stage.description}</p>
                            ...
                            <div className="relative mt-4 space-y-2">
                              {signals.map((signal) => (
                                <div
                                  ...
                                  data-workflow-indicator
                                  className={cn(
                                    "workflow-card-gpu relative overflow-hidden rounded-md border border-white/10 bg-black/18 px-3 py-2 transition-opacity duration-[800ms] ease-[cubic-bezier(0.215,0.61,0.355,1)]",
                                    visualState === "future" ? "opacity-60" : "opacity-100"
                                  )}
                                >
                                  <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <span className="min-w-0 flex-1 truncate text-[11px] font-medium text-current/78">
                                      {signal}
                                    </span>
                                    <span className="text-[10px] font-semibold uppercase tracking-[0.08em] text-current/48">
                            ...
                            <div className="relative mt-4 grid grid-cols-3 gap-2">
                              {metrics.map((metric) => (
                                <span
                                  ...
                                  data-workflow-indicator
                                  className={cn(
                                    "workflow-indicator-gpu rounded-md border border-white/10 bg-white/[0.055] px-2 py-2 text-center text-[10px] font-semibold uppercase tracking-[0.08em] text-current/72 transition-opacity duration-[800ms] ease-[cubic-bezier(0.215,0.61,0.355,1)]",
                                    visualState === "future" ? "opacity-50" : "opacity-100"
                                  )}
                                >
                                  {metric}
                                </span>
                            ...
                            <AnimatePresence>
                              {note ? (
                                <motion.p
                                  ...
                                  className="relative mt-4 rounded-md border border-white/10 bg-black/18 p-3 text-xs leading-5 text-current/72"
                                >
                                  {note}
                                </motion.p>
                              ) : null}
                                ...
                                {stage.responseDocuments.slice(0, 2).map((document) =>
                                  document.url ? (
                                    <a
                                      ...
                                      target="_blank"
                                      rel="noreferrer"
                                      className="flex items-center gap-2 rounded-md border border-white/10 bg-black/18 px-3 py-2 text-xs text-current/74 transition hover:text-white"
                                    >
                                    ...
                                    <span
                                      key={document.id || document.filename}
                                      className="flex items-center gap-2 rounded-md border border-white/10 bg-black/18 px-3 py-2 text-xs text-current/62"
                                    >
                                      <FileText className="h-3.5 w-3.5" />
                                      <span className="min-w-0 truncate">{document.filename}</span>
                                    </span>
