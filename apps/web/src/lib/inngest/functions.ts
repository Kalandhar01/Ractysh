...
export const scoreConsultationLead = inngest.createFunction(
  { id: "score-consultation-lead", name: "Score consultation lead" },
...
export const publishScheduledContent = inngest.createFunction(
  { id: "publish-scheduled-content", name: "Publish scheduled content" },
...
export const syncAnalyticsReports = inngest.createFunction(
  { id: "sync-analytics-reports", name: "Sync analytics reports" },
  { cron: "0 */6 * * *" },
  async ({ step }) => {
    return step.run("collect configured provider statuses", () => ({
      providers: ["posthog", "sentry", "vercel", "better-stack", "lighthouse"],
      syncedAt: new Date().toISOString()
    }));
  }
);

export const adminInngestFunctions = [scoreConsultationLead, publishScheduledContent, syncAnalyticsReports];
