...
function html(payload: {
  ...
  const tableRows = rows(payload)
    .map(
      ([label, value]) => `
        <tr>
          <td style="width:34%;padding:14px 16px;color:#7b6b51;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.09em;border-bottom:1px solid #eadfc9">${escapeHtml(label)}</td>
          <td style="padding:14px 16px;color:#181512;font-size:15px;font-weight:700;border-bottom:1px solid #eadfc9">${escapeHtml(value)}</td>
        </tr>
      `
    )
    .join("");
  ...
  return `
    <div style="margin:0;padding:0;background:#f7f1e5;color:#181512;font-family:Inter,Arial,sans-serif">
      <div style="padding:32px 16px">
        <div style="max-width:720px;margin:0 auto;overflow:hidden;border-radius:22px;border:1px solid #dcc99f;background:#fffaf0;box-shadow:0 28px 82px rgba(24,18,12,.16)">
          <div style="background:#11100e;padding:30px;color:#fff8ec">
            <p style="margin:0 0 9px;color:#d6b45f;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.15em">Ractysh enterprise desk</p>
            <h1 style="margin:0;font-size:26px;line-height:1.2;font-weight:700">New Ractysh Service Request</h1>
          </div>
...
async function sendServiceRequestEmail(payload: {
  ...
  return sendResendEmail({
    from: process.env.SERVICE_REQUEST_MAIL_FROM || process.env.MAIL_FROM || DEFAULT_FROM,
    to: parseEmailList(process.env.SERVICE_REQUEST_MAIL_TO || process.env.MAIL_TO, DEFAULT_RECIPIENT),
